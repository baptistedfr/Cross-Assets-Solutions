...
from Momentum.enums import FrequencyType
...
class Results:
    ...
    plt_rebalancing_plot : Union[go.Figure, list[go.Figure]] = None
    ...
    def get_statistics(self) -> pd.DataFrame:
        ...
        data = {
            "Metrics": ["Total Return", "Annualized Return", "Volatility", "Sharpe Ratio", "Sortino Ratio", "Max Drawdown", "VaR 95%", "CVaR 95%", "Frais de transaction"],
        ...
        for i, metric in enumerate(data[self.strategy_name]):
            ...
            elif i in [0, 1, 2, 5, 6, 7, 8]:  # Total Return, Annualized Return, Volatility, Max Drawdown (en pourcentage)
    ...
    def rebalancing_plot(self):
        if self.ptf_rebalancing is not None :            
            fig = go.Figure()
            fig.add_trace(go.Bar(
                x=self.ptf_rebalancing[self.ptf_rebalancing == 1].index,  # Dates de rebalancement
                y=[1] * len(self.ptf_rebalancing[self.ptf_rebalancing == 1]),  # Hauteur fixe pour les barres
                marker=dict(color='red'),  # Couleur des barres
                name="Rebalancing Periods"
            ))

            # Personnalisation du graphique
            fig.update_layout(
                title=f"Périodes de rabalancement de la stratégie : {self.strategy_name}",
                xaxis_title="Date",
                yaxis=dict(
                    showticklabels=False,  # Masquer les labels de l'axe Y
                    range=[0, 1.2]
                ),
                showlegend=True
            )

            # Stocker le graphique pour éviter de le recalculer
            self.plt_rebalancing_plot = fig
    ...
    def compare_results(results : list["Results"]) -> "Results":
        ...
        rebalancing_plots = Results.combine_rebalancing_plot(results)
        ...
        return Results(ptf_values=results[0].ptf_values, ptf_weights=results[0].ptf_weights,
                       ...
                       ptf_weights_plot=weight_plots,  plt_rebalancing_plot = rebalancing_plots,
    ...
    @staticmethod
    def combine_rebalancing_plot(results : list["Results"]) -> go.Figure:
        """
        Combine the rebalancing plots.
        As the function is also used to compare with the benchmark, we want to return only one weight plot if it's the case else we return both.
        ...
        rebalancing_plots = []
        for result in results:
            if result.plt_rebalancing_plot is not None:
                rebalancing_plots.append(result.plt_rebalancing_plot)
    

        return rebalancing_plots